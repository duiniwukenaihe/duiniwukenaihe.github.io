...
## 描述背景：
    ...
    早前公司代码管理采用的docker compose搭建的gitlab管理。为完成资源优化，高可用。现在将环境迁移到kubernetes集群环境中。
    compose 文件基本是基于https://raw.githubusercontent.com/sameersbn/docker-gitlab/master/docker-compose.yml进行修改的，修改了gitlab images twang2218/gitlab-ce-zh:latest 中文版镜像.
    gitlab  postgresql  redis都挂载本地目录为/data/docker/相对应目录下 端口映射80映射到主机10080  22 映射到10022主机端口。
...
修改gitlab.yaml 文件gitlab image 版本号。

kubectl apply gitlab.yaml 
